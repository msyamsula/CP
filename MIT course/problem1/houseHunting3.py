...
class HouseHunting():
    ...
    def calculate(self):
        z=1+(0.04/12)
        gamma=1+z+z**2+z**3+z**4+z**5
        y=1+self.__prices[2]
        largeNumber=(z**30)+y*(z**24)+(y**2)*(z**18)+(y**3)*(z**12)+(y**4)*(z**6)+(y**5)
        print(self.getDPneeded())
        self.__bestSaving=self.getDPneeded()/(largeNumber*self.__prices[0])
    ...
    def start(self):
        ...
        print('Recommended saving ratio: ', self.__bestSaving)
